...
func CreateFFmpegCommand(oc config.OverlayConfig, sc config.StreamConfig) *exec.Cmd {
	...
	cmdArgs := []string{
		"-i", sc.Src,
		"-i", oc.LogoPath,
		"-filter_complex", fmt.Sprintf("[1:v]format=rgba,colorchannelmixer=aa=%f[logo];[logo][0:v]scale=w=%d*(rh/%d):h=%d*(rh/%d)[logo_scaled];[0:v][logo_scaled]overlay=(main_w*0.035):(main_h-overlay_h)/2", oc.LogoAlpha, oc.LogoWidth, referenceHeight, oc.LogoHeight, referenceHeight),
		"-c:v", "libx264",
		"-preset", "ultrafast",
		"-tune", "zerolatency",
		"-bf", "0",
